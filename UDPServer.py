...
#so with each dictionary, it will have depart and final destination
#There may be duplicates, so say 1. Bellville to 2. Nyanga and 2. Nyanga to bellville
...
while True:
    #here we will recieve the departure and destination from the client
    depart, clientAddress = serverSocket.recvfrom(2048)
    dest, clientAddress = serverSocket.recvfrom(2048)
    ...
    serverSocket.sendto(modifiedMessage1.encode(), clientAddress)
